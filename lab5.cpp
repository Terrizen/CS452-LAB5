// Code Compiles but the texture doesn't load
...
void init(){
	...
	int width, height;
	unsigned char* image =
    		SOIL_load_image("Dream_Sword", &width, &height, 0, SOIL_LOAD_RGB);
	...
	*/
	...
	int width, height;
	unsigned char* image =
    		SOIL_load_image("Dream_Sword.png", &width, &height, 0, SOIL_LOAD_RGB);
	assert(image != NULL);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB,
              	GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
